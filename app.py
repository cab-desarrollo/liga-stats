...
def cargar_indice_partidos() -> Tuple[Optional[pd.DataFrame], List, List, List, List, List, List, Optional[str]]:
    ...
    Devuelve: DataFrame procesado, listas para filtros y un string de error si falla,
    ...
    try:
        ...
        # 2. Equipos
        if 'LOCAL/VISITANTE' not in df_indice.columns:
...
(df_indice,
 LISTA_ANIOS,
 LISTA_COMPETENCIAS,
 LISTA_CATEGORIAS_FULL,
 LISTA_FASES_FULL,
 LISTA_GRUPOS_FULL,
 LISTA_EQUIPOS,
 error_carga) = cargar_indice_partidos()
...
with st.expander("🔍 Buscar un Partido para copiar ID"):
    ...
    if 'df_indice' in locals() and not df_indice.empty:
        ...
        # --- FILTROS ---
        st.write("Filtra para encontrar el ID del partido:")
        col1, col2, col3 = st.columns(3)

        with col1:
            # Filtro 1: Año
            sel_anio = st.selectbox("Año", ["TODOS"] + LISTA_ANIOS, key="sel_anio")
            if sel_anio != "TODOS":
                df_filtrado = df_filtrado[df_filtrado['Año'] == sel_anio]

            # Filtro 4: Equipo
            sel_equipo = st.selectbox("Equipo", ["TODOS"] + LISTA_EQUIPOS, key="sel_equipo")
            if sel_equipo != "TODOS":
                df_filtrado = df_filtrado[
                    (df_filtrado['Equipo_Local'] == sel_equipo) |
                    (df_filtrado['Equipo_Visitante'] == sel_equipo)
                ]

        with col2:
            # Filtro 2: Competencia (depende de Año)
            opciones_competencia = ["TODAS"] + sorted(df_filtrado['COMPETENCIA'].astype(str).unique())
            sel_competencia = st.selectbox("Competencia", opciones_competencia, key="sel_comp")
            if sel_competencia != "TODAS":
                df_filtrado = df_filtrado[df_filtrado['COMPETENCIA'] == sel_competencia]

            # Filtro 5: Condición (depende de Equipo)
            sel_condicion = st.selectbox(
                "Condición",
                ["CUALQUIERA", "LOCAL", "VISITANTE"],
                key="sel_cond",
                disabled=(sel_equipo == "TODOS") # Se activa solo si se elige un equipo
            )
            # Aplicar filtro de condición solo si se eligió un equipo
            if sel_equipo != "TODOS" and sel_condicion != "CUALQUIERA":
                if sel_condicion == "LOCAL":
                    df_filtrado = df_filtrado[df_filtrado['Equipo_Local'] == sel_equipo]
                elif sel_condicion == "VISITANTE":
                    df_filtrado = df_filtrado[df_filtrado['Equipo_Visitante'] == sel_equipo]

        with col3:
            # Filtro 3: Categoría (depende de Año y Competencia)
            opciones_categoria = ["TODAS"] + sorted(df_filtrado['CATEGORÍA'].astype(str).unique())
            sel_categoria = st.selectbox("Categoría", opciones_categoria, key="sel_cat")
            if sel_categoria != "TODAS":
                df_filtrado = df_filtrado[df_filtrado['CATEGORÍA'] == sel_categoria]

        # --- FILTROS DEPENDIENTES ADICIONALES (Fase y Grupo) ---
        col4, col5 = st.columns(2)
        with col4:
            # Filtro 6: Fase (depende de los filtros anteriores)
            opciones_fase = ["TODAS"] + sorted(df_filtrado['FASE'].astype(str).unique())
            sel_fase = st.selectbox("Fase", opciones_fase, key="sel_fase")
            if sel_fase != "TODAS":
                df_filtrado = df_filtrado[df_filtrado['FASE'] == sel_fase]

        with col5:
            # Filtro 7: Grupo (depende de los filtros anteriores)
            opciones_grupo = ["TODAS"] + sorted(df_filtrado['GRUPO'].astype(str).unique())
            sel_grupo = st.selectbox("Grupo", opciones_grupo, key="sel_grupo")
            if sel_grupo != "TODAS":
                df_filtrado = df_filtrado[df_filtrado['GRUPO'] == sel_grupo]

        # --- RESULTADOS DEL FILTRO ---
        st.markdown("---")
        st.subheader(f"Partidos Encontrados ({len(df_filtrado)})")
        st.info("Copia el 'ID' del partido que quieras y pégalo en el campo 'ID de partido' de abajo.")

        # Mostrar la tabla de resultados
        df_mostrar = df_filtrado[['FECHA', 'LOCAL/VISITANTE', 'ID', 'COMPETENCIA', 'CATEGORÍA', 'FASE']]
        # Ordenar por fecha más reciente primero
        st.dataframe(df_mostrar.sort_values(by='FECHA', ascending=False), use_container_width=True, height=300)

    else:
        st.warning("El archivo de índice 'partidos/designacionesfecha-excel.xlsx' no se pudo cargar. El buscador no está disponible.")
